...
def test_horizontals_master110page19(td, mp):
    ...
    lines."""
    cmd = f'-i {hoverpower.MASTER110_PDF} --pages=19 --line --horizontals --annotation'
    tests.run(cmd, mp=mp)
    horizontals = serializeraw.load_horizontals(td.tmpdir)
    horizontals = utilo.select_content(horizontals, page=19)
    assert len(horizontals) == 2, str(horizontals)


def test_horizontals_master155page1(td, mp):
    cmd = f'-i {hoverpower.MASTER155_PDF} --pages=1 --line --horizontals --annotation'
    tests.run(cmd, mp=mp)
    horizontals = serializeraw.load_horizontals(td.tmpdir)
    horizontals = utilo.select_content(horizontals, page=1)
    assert len(horizontals) == 1, str(horizontals)


def test_horizontals_rotated_master116(td, mp):
    cmd = f'-i {hoverpower.MASTER116_PDF} --pages=102 --line --horizontals'
    tests.run(cmd, mp=mp)
...
def test_horizontals_not_rotated_bachelor026p16(td, mp):
    ...
    cmd = f'-i {hoverpower.BACHELOR026_PDF} --pages=16 --line --horizontals'
    tests.run(cmd, mp=mp)
...
def test_lines_bachelor63page10(td, mp):
    cmd = f'-i {hoverpower.BACHELOR063_PDF} --pages=10 --line --annotation'
    tests.run(cmd, mp=mp)
    lines = serializeraw.load_lines(iamraw.path.line(td.tmpdir))
    lines = utilo.select_content(lines, page=10)
    assert len(lines) == 46


def test_lines_master72_hyperlinks_as_line(td, mp):
    cmd = f'-i {hoverpower.MASTER072_PDF} --pages=65 --line --annotation'
    tests.run(cmd, mp=mp)
...
def test_lines_master75_hyperlinks_as_line(td, mp):
    cmd = f'-i {hoverpower.MASTER075_PDF} --pages=6 --line --annotation'
    tests.run(cmd, mp=mp)
...
def test_lines_bachelor032p3(td, mp):
    ...
    technique.
    """
    cmd = f'-i {hoverpower.BACHELOR032_PDF} --pages=3 --line'
    tests.run(cmd, mp=mp)
    lines = serializeraw.load_lines(td.tmpdir)[0].content
