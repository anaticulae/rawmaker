...
def extract(source, pages, td, mp) -> list:
    ...
    if not os.path.exists(td.tmpdir.join('rawmaker__images_images')):
        return []
    extracted = utilo.file_list('rawmaker__images_images', include='png')
    return extracted
...
def test_image_extract_with_pages_offset(td, mp):
    ...
    cmd = f'-i {hoverpower.MASTER116_PDF} --images --pages=16:20'
    tests.run(cmd, mp=mp)
...
def test_render_master75page0_10_28(mp, td):
    ...
    extracted correctly."""
    extracted = extract(hoverpower.MASTER075_PDF, '0:10,28', td, mp)
    expected = 4
...
def test_render_master127page32(mp, td):
    ...
    extracted = extract(hoverpower.MASTER127_PDF, 32, td, mp)
...
def test_image_master31page10(mp, td):
    ...
    extracted = extract(hoverpower.MASTER031_PDF, 10, td, mp)
    assert len(extracted) == 1


def test_image_master31page4(mp, td):
    """Do not detect anything as image"""
    extracted = extract(hoverpower.MASTER031_PDF, 4, td, mp)
    assert not extracted


@pytest.mark.skip(reason='investigate bounding behavior')
def test_image_master105(mp, td):
    # extracted = extract(hoverpower.MASTER105_PDF, '89,90,91', td, mp)
    path = os.path.join(hoverpower.generated('shorten'), 'master105.pdf')
