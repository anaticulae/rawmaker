...
def test_math_intersecting_lines(first, second, expected):
    ...
    match = linero.math.intersecting_lines(first, second)
...
def test_math_intersecting_line_invalid_input(first, second):
    with pytest.raises(ValueError):
        linero.math.intersecting_lines(first, second)
...
def test_math_intersecting_line_with_offset():
    ...
    with_error = linero.math.intersecting_lines(first, second, max_diff=5.0)
    assert with_error
