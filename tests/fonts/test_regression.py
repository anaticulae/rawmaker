...
def test_leftright_book_font_name(mp, capsys):
    tests.run(
        f'-i {hoverpower.BOOK007_PDF} --fonts',
        mp=mp,
    )
...
def test_bachelor90_fontname(mp, capsys):
    ...
    """
    tests.run(
        f'-i {hoverpower.BACHELOR090_PDF} --fonts --pages=1:20',
        mp=mp,
    )
    stderr = utilotest.stderr(capsys)
    assert 'ERROR' not in stderr
...
def test_leftright_book_font_size(td, mp):
    tests.run(
        f'-i {hoverpower.BOOK007_PDF} --text',
        mp=mp,
    )
    source = iamraw.path.text(td.tmpdir)
    position = serializeraw.load_document(source)
    first_page = utilo.select_page(position, page=0)
...
def test_porting_module_font_index(strip, td, mp):
    ...
    cmd = (f'-i {hoverpower.DOCU009_PDF} --fonts --text {nostrip}')
    tests.run(cmd, mp=mp)
...
@pytest.mark.parametrize('pdf, strip', [
    pytest.param(hoverpower.MASTER072_PDF, True, id='master72_true'),
    pytest.param(hoverpower.MASTER072_PDF, False, id='master72_false'),
])
@utilotest.nightly
...
def test_arabic_fonts(mp):
    ...
    cmd = f'-i {hoverpower.DISS272_PDF} --fonts --pages=246:251'
    tests.run(cmd, mp=mp)
...
def test_font_size_huge_master193(mp):
    cmd = f'-i {hoverpower.MASTER193_PDF} --fonts --text --pages=2'
    tests.run(cmd, mp=mp)
...
def test_font_bold_bachelor067page59(td, mp):
    cmd = f'-i {hoverpower.BACHELOR067_PDF} --text --fonts  --pages=59'
    tests.run(cmd, mp=mp)
...
def test_font_underline_bachelor28p16(td, mp):
    cmd = f'-i {hoverpower.BACHELOR028_PDF} --text --line --horizontals --pages=16'
    tests.run(cmd, mp=mp)
    ptcn = serializeraw.ptn_frompath(td.tmpdir)
